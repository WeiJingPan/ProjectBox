...
public class JsonTool
{
    ...
    public static object JsonDecode(string json)
    {
        ...
        if (json != null)
        {
            ...
            int index = 0;
            bool success = true;
            object value = JsonTool.parseValue(charArray, ref index, ref success);

            if (success)
                JsonTool.lastErrorIndex = -1;
            else
                JsonTool.lastErrorIndex = index;
    ...
    public static string jsonEncode(object json)
    {
        ...
        var success = JsonTool.serializeValue(json, builder);
    ...
    {
        return (JsonTool.lastErrorIndex == -1);
    }
    ...
    {
        return JsonTool.lastErrorIndex;
    }
    ...
    {
        if (JsonTool.lastErrorIndex == -1)
        {
            return "";
        }
        else
        {
            int startIndex = JsonTool.lastErrorIndex - 5;
            int endIndex = JsonTool.lastErrorIndex + 15;
            if (startIndex < 0)
                startIndex = 0;

            if (endIndex >= JsonTool.lastDecode.Length)
                endIndex = JsonTool.lastDecode.Length - 1;

            return JsonTool.lastDecode.Substring(startIndex, endIndex - startIndex + 1);
        }
    }
